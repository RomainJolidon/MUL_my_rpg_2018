...
void fire_animation(game_t *game, int *event)
{
    if (game->c->first == NULL)
        fire_effect(game, game->scene->info_fight->target->e->pos.x + 60 \
        , game->scene->info_fight->target->e->pos.y + 60);
    if (event[0] % 2 == 0)
...
{
    game->scene->info_fight->to_do = do_fire;
    game->scene->info_fight->anim = fire_animation;
    game->scene->state = 5;
}
